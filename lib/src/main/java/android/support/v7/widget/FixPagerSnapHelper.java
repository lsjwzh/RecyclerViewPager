 ...
 */
public abstract class FixPagerSnapHelper extends SnapHelper {
    ...
    @Nullable
    protected View mCenterViewWhenTouchDown;

    public void updateTouchStartView(RecyclerView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
            mCenterViewWhenTouchDown = findCenterView(layoutManager, getVerticalHelper(layoutManager));
        } else if (layoutManager.canScrollHorizontally()) {
            mCenterViewWhenTouchDown = findCenterView(layoutManager, getHorizontalHelper(layoutManager));
        }
    }
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX,
        ...
        if (mCenterViewWhenTouchDown == null) {
            return RecyclerView.NO_POSITION;
        }
        final int centerPosition = layoutManager.getPosition(mCenterViewWhenTouchDown);
        ...
        int targetPosition = reverseLayout
                ...
                : (forwardDirection ? centerPosition + 1 : centerPosition - 1);
        RecyclerView.Adapter adapter = layoutManager.mRecyclerView.getAdapter();
        int maxPosition = adapter == null ? targetPosition : adapter.getItemCount() - 1;
        int minPosition = 0;
        targetPosition = Math.min(maxPosition, targetPosition);
    ...
    public abstract void onSmoothScrollStop();
